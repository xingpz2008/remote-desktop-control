...
public class MouseListener extends MouseAdapter {
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        LOGGER.debug("mouse released:{}", mouseEvent);
        sendMosueEvent(mouseEvent);
    }
    ...
    private void MoveMouseIfNecessary(MouseEvent e) {
        if (Math.abs(lastPoint.x - e.getX()) > 2 || Math.abs(lastPoint.y - e.getY()) > 2) {
            lastPoint.x = e.getX();
            lastPoint.y = e.getY();
            //先移动
            mouseMoved(e);
